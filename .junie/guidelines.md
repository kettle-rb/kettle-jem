Project: kettle-jem — Development Guidelines (for advanced contributors)
...
- Gem signing (for releases)
  ...
  - Important for local testing (to avoid hanging prompts): ALWAYS skip signing when building locally to test the packaging or install process. Without the private key password, the build will wait indefinitely at a signing prompt.
    - One-off commands (recommended):
      - SKIP_GEM_SIGNING=true gem build kettle-jem.gemspec
...
- Running tests (verified)
  ...
  - Focused runs
    ...
    - Example: K_SOUP_COV_MIN_HARD=false bin/rspec spec/kettle-jem/class_spec.rb:42
  - Output visibility
...
- Adding new tests (guidelines)
  ...
  - Spec file names must map to a real class or module under lib/ (mirror the path). Do not introduce specs for non-existent classes or ad-hoc names (e.g., avoid template_helpers_replacements_spec.rb when testing Kettle::Jem::TemplateHelpers; add those examples to template_helpers_spec.rb).
...
- Clarification: Executable scripts provided by this gem (exe/* and installed binstubs) work when the gem is installed as a system gem (gem install kettle-jem). However, the Rake tasks provided by this gem require kettle-jem to be declared as a development dependency in the host project's Gemfile and loaded in the project's Rakefile.
